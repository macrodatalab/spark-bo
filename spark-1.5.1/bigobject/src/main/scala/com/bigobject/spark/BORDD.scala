...
object BORDD extends Logging {
  ...
  def sql(
    ...
    var allRdd = null.asInstanceOf[RDD[InternalRow]]
    ...
    // Note: CAN't directly transform InternalRow to Row, b/c converter will throw exception
	//       Not like BORelation, set needConversion = false to skip converter
    val schemaTypes = schema.fields.map(x => x.dataType)
    val rowRDD = allRdd.map{ir: InternalRow => Row.fromSeq(ir.toSeq(schemaTypes))}
    sqlSc.createDataFrame(rowRDD, schema)
  }
        ...
        for (i <- 0 until fields.size) {
          fields(i).dataType match {
            case DateType =>
              ...
              if (dateVal != null) {
                mutableRow.update(i, DateTimeUtils.millisToDays(DateTimeUtils.stringToTime(dateVal).getTime()))
              } else {
